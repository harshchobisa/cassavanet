...
from .networks_generic import Generic_Net
from .networks_resnet_tuning import Resnet_Tuning_Net
...
def train(
    ...
    # Create dataloaders
    train_dataset = DailyBruinDataset(train_df, network_number)
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=batch_size, shuffle=True
    )

    val_dataset = DailyBruinDataset(val_df, network_number)
    val_dataloader = torch.utils.data.DataLoader(
        val_dataset, batch_size=batch_size, shuffle=True
    )

    # Create model
    networks = [Generic_Net, Resnet_Tuning_Net]

    Net = networks[network_number]
...
def evaluate(model, val_dataloader, summary_writer, step, device):
    ...
    with torch.no_grad():
        ...
        #         torch.cat([first_images], dim=0), nrow=2
        #     ),
        #     global_step=step,
        # )
